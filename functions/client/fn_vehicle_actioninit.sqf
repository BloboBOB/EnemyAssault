...
while {true} do 
{
	...
	
	{
		...
		{
			ghst_sling_Array = ghst_sling_Array + [_x];
			_x addAction ["<t  shadow='1' color=""#ffff00"">Slingload Vehicle</t>", "call ghst_fnc_slingload", [], 1, false, true, "","alive _target and _this == driver _target"];
		};
		if (((typeOf _x) in (ghst_Supported_Vehicles_Boat)) and !(_x in ghst_boat_Array)) then
		{
			ghst_boat_Array = ghst_boat_Array + [_x];
			_x addAction ["<t  shadow='1' color=""#FF9900"">Push Boat</t>", "call ghst_fnc_BoatPush", [], 1, false, true, "", "_this distance _target < 8 and !(_this in _target)"];  
		};
		if (((typeOf _x) in (ghst_Supported_AmmoBoxes)) and !(_x in ghst_AmmoBoxes_Array)) then
		{
			ghst_AmmoBoxes_Array = ghst_AmmoBoxes_Array + [_x];
			_x addaction ["<t  shadow='1' color='#ffffff'>Open Virtual Arsenal</t>", { ["Open",true] call BIS_fnc_arsenal; }, [], 1, false, true, "","alive _target"];
			_x addAction["<t  shadow='1' color='#ffa200'>Virtual Ammobox</t>", "VAS\open.sqf", [], 1, false, true, "","alive _target"];
		};
