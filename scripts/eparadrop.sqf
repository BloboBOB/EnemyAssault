/*
V1.8 script by: Ghost - Creates specifed air vehicle and flys to Objective Area and drops enemy paras.
...
while {_loop} do {
	_spawnpos = [(_objpos select 0) + (_radx * (sin(random 360))),(_objpos select 1) + (_rady * (cos(random 360))), _flyheight];
		if ((_spawnpos distance _objpos) > (_radx - 200)) exitwith {_loop = false;};
};
...
[_paras1,_objpos, _radarray,_markunitsarray,["COMBAT", "FULL", "WEDGE"]] call fnc_ghst_waypoints;
	...
	eparadrop = [_objpos,_air_type,_radarray,_paras1,_repeat,_markunitsarray,aiSkill] spawn {
		...
		deletegroup _paras1;
			if (_repeat) then {
				eparadrop = [_objpos,_radarray,_air_type,_repeat,_markunitsarray,_aiSkill] execVM "scripts\eparadrop.sqf";
			};
		if (true) exitwith {};
	};
