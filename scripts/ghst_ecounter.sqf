/*V3.5.2 by: Ghost - create enemy convoy for a counter offensive.
	...
	while {_loop} do {
		...
		_roads = (_startpos nearRoads 200);
			if !(isnil "_roads") then {
				if (((_startpos distance _position_mark) > (_radx - 200)) and !(count _roads == 0)) exitwith {_loop = false;};
			};
	};
...
for "_e" from 0 to (_enum)-1 do {
	...
	[_eGrp,_position_mark, [_rad,_rad],_markunitsarray,["AWARE", "NORMAL", "WEDGE"]] call fnc_ghst_waypoints_unload;
